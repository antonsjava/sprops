 ...
 * @author antons
 */
public class FileEncodeCommand implements Command {
    ...
    public boolean checkInput(Args args) {
        String file = args.first("-fencode", "--fencode");
        if(Is.empty(file)) {
        ...
        if(!args.isSet("")) {
            log.warning("No properties to encode are provided");
            List<String> list = propfile.propsToEncode();
            log.warning("You can encode properties");
        ...
        for(String string : strings) {
            try {
                ...
                if(value.startsWith("sprops:")) {
                    log.info("Property: '"+string+"' already encoded");
                    continue;
                }
                String encoded = encoder.encode(value);
                propfile.replace(string, encoded);
